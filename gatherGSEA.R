###Collect GSEA results.  Assumes a limited gene set across a large number of samples all in the same GSEA output folder.
...
##edit string from folder names to get report names (xls file name is the same as the sheet name, minus a few digits)
...
allsamples$NOM.p.val = allsamples$NOM.p.val + 0.0001
...
allsamples$NAME <- factor(allsamples$NAME, levels = c("HALLMARK_APOPTOSIS","GRYDER_HOUSEKEEPING","TFS_NO_EPIMACHINES","GRYDER_RH4_SE_GENES","GRYDER_RH4_TOP_SE_TFS"))

library(ggplot2)
ggplot(allsamples, aes(x = Drug, y = NAME, size = abs(ES)-ES.min,colour = ES)) + geom_point() +scale_colour_gradient2(low="red3",mid="grey", high="deepskyblue3") +
 theme(axis.text.x = element_text(angle = 90, hjust = 1))

ggplot(subset(allsamples,!(allsamples$NAME %in% c("GRYDER_HOUSEKEEPING"))), aes(x = Drug, y = NAME, size = abs(ES)-ES.min,colour = ES)) + geom_point() +scale_colour_gradient2(low="blue", mid="grey", high="red") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

#extract out RANK and RUNNING ES data for a given gene set
Geneset = "GRYDER_RH4_TOP_SE_TFS"
...
library(gridExtra)
library(wesanderson)
Drugcolors = c("deepskyblue3","indianred1","red3")
